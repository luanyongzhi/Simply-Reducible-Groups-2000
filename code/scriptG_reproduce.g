# Reproduction copy, September 8, 2026.
# Original scriptG.g is preserved unchanged.
# Only change: the two ANUPQ resource options Workspace are spelled
# PqWorkspace, as documented by ANUPQ. The search constraints,
# SR filter, checkpoint logic and descendant reader are unchanged.
# Start in a clean directory containing SR_groups_results512.csv. To reproduce the
# completed run from scratch, do not supply an old scriptG_results.log.
#############################################################################
##
...
RunJob := function(k, id, s)
  ...
  res := CALL_WITH_CATCH(function()
           return PqDescendants(P : StepSize := s, PqWorkspace := PQWORKSPACE);
         end, []);
  if res[1] <> true then                     # fallback: standard presentation
    res := CALL_WITH_CATCH(function()
             local S;
             S := PcGroupFpGroup(StandardPresentation(P));
             return PqDescendants(S : StepSize := s, PqWorkspace := PQWORKSPACE);
           end, []);
  fi;
